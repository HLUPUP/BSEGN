...
class Trainer(BaseTrainer):
    ...
    def test(self):
        ...
        # -- [ TEST Image Directory ] -- #
            ...
            self.test_dir_wave(self.cfg['test_dir'])
            exit()
        # -- [ TEST DND Benchmark ] -- #
            ...
            exit()
        # -- [ Test Normal Dataset ] -- #
        elif self.cfg['builduct'] is not None:
            
            self.builduct_dynamic_all_bin()
    ...
    def _forward_fn(self, module, loss, data):
        # forward
        input_data = [data['dataset'][arg] for arg in self.cfg['model_input']] #hl 24x3x120x120
        denoised_img = module['denoiser'](*input_data) # 24 3 120 120
        model_output = {'recon': denoised_img}

        # get losses
        
        losses, tmp_info,cf_num, cf_num_1,cf_num_2= loss(input_data, model_output, data['dataset'], module, \
                                    ratio=(self.epoch-1 + (self.iter-1)/self.max_iter)/self.max_epoch,epoch=self.epoch)
