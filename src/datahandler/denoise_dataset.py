...
class DenoiseDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #data = self._pre_processing(data) hl
    ...
    def _load_wave( self,img_name, as_gray=False):
        # signalFile_base       = h5py.File(img_name, 'r')
        signalFile_base       = scipy.io.loadmat(img_name)
        sigMat      = np.array(signalFile_base['pre_beamform_data'])#wave_all_re
        # 
        # sigMat = np.swapaxes(sigMat, 0,1)
        
        
        # sigMat = np.concatenate([sigMat[:,i::40] for i in range(40)],axis=1)
        # sigMat = np.transpose(sigMat, (2,0,1))
        # sigMat=sigMat[:,np.newaxis,:]
        # sigMat = np.tile(sigMat[:, :, np.newaxis], (1, 1, 512))
        
        # sigMat=np.concatenate([sigMat,sigMat],axis=1)
